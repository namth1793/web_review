...
import {
  ...
  getAdminCategories, getAdminAuthors
} from '../../api/admin'
  ...
  useEffect(() => {
    ...
    getAdminPost(id).then(post => {
      setForm({
        ...
        pros: parseJsonField(post.pros).concat(['', '', '']).slice(0, Math.max(parseJsonField(post.pros).length, 3)),
        cons: parseJsonField(post.cons).concat(['', '', '']).slice(0, Math.max(parseJsonField(post.cons).length, 3)),
        review_highlights: parseJsonField(post.review_highlights)
          .concat([{ label: '', value: '' }, { label: '', value: '' }, { label: '', value: '' }])
          .slice(0, Math.max(parseJsonField(post.review_highlights).length, 3))
      })
    ...
    <div className="p-8 max-w-5xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">{isEdit ? `Editing ID #${id}` : 'Create a new review article'}</p>
        </div>
        <div className="flex gap-3">
          <button
            type="button"
            onClick={() => setPreview(p => !p)}
            className="text-gray-400 hover:text-white border border-gray-700 px-4 py-2 rounded-lg text-sm transition hover:border-gray-500"
          >
      ...
      {preview ? (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-8">
          <h2 className="text-white text-2xl font-bold mb-2">{form.title}</h2>
          <p className="text-gray-400 text-sm mb-6">{form.excerpt}</p>
          <div
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Product Name Review 2026: Compelling Subtitle Here"
                required
              />
            </div>
            <div className="flex gap-3">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="A compelling 1–2 sentence summary shown in cards and meta description..."
              />
            </div>
            <div className="flex gap-3">
              <div className="flex-1">
            ...
            <div>
              <label className="block text-gray-300 text-xs font-medium mb-1.5">Featured Image URL</label>
              <input
                value={form.featured_image}
                onChange={e => set('featured_image', e.target.value)}
                className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-red-500 transition"
                placeholder="https://picsum.photos/seed/yourproduct/800/500"
              />
            </div>
          ...
          {/* Links */}
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
            <h2 className="text-white font-semibold text-sm uppercase tracking-wide">Links & CTA</h2>
            <div>
            ...
            <div className="flex gap-3">
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-red-500 transition"
                  placeholder="https://productname.com/signup?ref=insignreview"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-white font-semibold text-sm uppercase tracking-wide">Article Content (HTML)</h2>
              <span className="text-gray-500 text-xs">Supports full HTML</span>
            </div>
            <textarea
              value={form.content}
              onChange={e => set('content', e.target.value)}
              rows={18}
              className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2.5 text-sm font-mono focus:outline-none focus:border-red-500 transition resize-y"
              placeholder="<p>Write your review content here using HTML...</p>&#10;&#10;<h2>Section Heading</h2>&#10;<p>Content paragraph...</p>"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {['pros', 'cons'].map(field => (
              <div key={field} className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                <h2 className="text-white font-semibold text-sm uppercase tracking-wide mb-3 capitalize">{field}</h2>
                  ...
                  {form[field].map((item, i) => (
                    <div key={i} className="flex gap-2">
                      <input
                        ...
                        placeholder={`${field === 'pros' ? 'Pro' : 'Con'} ${i + 1}`}
                      />
                      <button
                        type="button"
                        onClick={() => removeListItem(field, i)}
                        className="text-gray-600 hover:text-red-400 text-sm w-6"
                      >×</button>
                    </div>
                  ))}
                </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            <h2 className="text-white font-semibold text-sm uppercase tracking-wide mb-3">Review Highlights (Key Stats)</h2>
            <div className="space-y-2">
              {form.review_highlights.map((h, i) => (
                <div key={i} className="flex gap-3">
                  <input
                    ...
                    className="flex-1 bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-red-500 transition"
                    placeholder="Label (e.g. Starting Price)"
                  />
                  <input
                    value={h.value}
                    onChange={e => setHighlight(i, 'value', e.target.value)}
                    className="flex-1 bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-red-500 transition"
                    placeholder="Value (e.g. $29/month)"
                  />
                  <button
                    type="button"
                    onClick={() => removeListItem('review_highlights', i)}
                    className="text-gray-600 hover:text-red-400 text-sm w-6"
                  >×</button>
                </div>
              ))}
            ...
            <button
              type="button"
              onClick={() => addListItem('review_highlights', { label: '', value: '' })}
              className="mt-3 text-gray-500 hover:text-gray-300 text-xs"
            >
              + Add highlight
            </button>
